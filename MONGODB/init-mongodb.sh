...
#sed -i 's|#oplogSize=10240|oplogSize=10240|' /usr/local/mongodb/mongodb-linux-x86_64-rhel70-3.6.9/mongodb.conf
...
#2立即导出日志：mongodump -u admin -p Adminqwe123 -h "rs0/192.168.8.50,192.168.8.51" -d local -c oplog.rs -o mongodbback/config
#3找到发生误删除的时间点
#bsondump oplog.rs.bson |egrep "\"op\":\"d\"\,\"ns\":\"test\.oplog\"" |head -1 "t":1515379110,"i":1
#4复制oplog到备份目录
#cp  mongodbback/config/oplog/oplog.rs.bson   mongodbback/config/oplog.bson
#5进行恢复，添加之前找到的误删除的点（limt）
#mongorestore -u admin -p Adminqwe123 --oplogReplay --oplogLimit "1515379110:1"  mongodbback/config
